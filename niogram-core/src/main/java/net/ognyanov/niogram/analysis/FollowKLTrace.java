...
/**
 * A trace for a terminal in the FollowKL set of a node.
 *
...
public class FollowKLTrace
{
    ...
    private List<FollowKLTrace> children = new ArrayList<FollowKLTrace>();

    private FollowKLTrace(GrammarNode node, int terminalType, int position,
     ...
     */
    public FollowKLTrace(GrammarNode node, int terminalType, int position)
    {
     ...
     * @return the parent
     */
    public FollowKLTrace getParent()
    {
        return parent;
    }
     ...
     */
    public List<FollowKLTrace> getChildren()
    {
        return children;
    }
    ...
    private void buildChildren()
    {
        if (node instanceof Grammar) {
            for (NonterminalRule rule : ((Grammar) node)
                .getNonterminalRules()) {
                if (rule.getFollowKL().get(position).get(terminalType)) {
                    FollowKLTrace child =
                        new FollowKLTrace(rule, terminalType, position);
                    child.parent = this;
                    children.add(child);
                }
            }
            ...
            FollowKLTrace current = this;
            for (Term term : terms) {
                if (term.getFollowKL().get(position).get(terminalType)) {
                    if (term instanceof Terminal) {
                        // the end of the road
                        if (term.getType() == terminalType) {
                            FollowKLTrace child =
                                new FollowKLTrace(term, terminalType, position,
                                                  false);
                        ...
                        if (term.getFollowKL().get(position)
                            .get(terminalType)) {
                            FollowKLTrace child =
                                new FollowKLTrace(term, terminalType, position,
                                                  false);
                    ...
                    FollowKLTrace child =
                        new FollowKLTrace(term, terminalType,
                                          position, false);
                    child.parent = current;
                    current.children.add(child);
                    current = child;
